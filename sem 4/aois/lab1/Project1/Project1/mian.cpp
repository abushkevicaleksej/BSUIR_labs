...
const vector <bool> minus_one_сomplementary = { 1, 1, 1, 1, 1, 1, 1, 1 };
...
vector <bool> to_binary_сomplementary(int num)
{
...
vector <bool> from_сomplementary_to_inverse(vector <bool> bin)
{
	if (bin[0] == 1)
	{
		bin = add(bin, minus_one_сomplementary);
		return bin;
	}
	else return bin;
}
...
vector <bool> from_straight_to_сomplementary(vector <bool> bin)
{
...
vector <bool> multiply(vector <bool> a, vector <bool> b)
{
	...
	{
		res = add(a, res);
		b = add(b, minus_one_сomplementary);
	}
	return res;
}
void division(vector <bool> a, vector <bool> b)
{
	...
	a[0] = 0;
	b[0] = 1;
	b = from_straight_to_сomplementary(b);
	while (1)
	{
...
binary_float from_float_to_binary_part1(float number)
{
	...
	if (integer_bin == binary_zero)
	{
		while (integer_bin != binary_one)
		{
			...
			res.index = add(res.index, minus_one_сomplementary);
		}
	}
	while (integer_bin != binary_one)
	{
...
{
	if (add(a.index, multiply(b.index, minus_one_сomplementary))[0] == 1)
	{
		return 1;
	}
	else
...
int main()
{
	...
	case 1:
		...
		switch (input2) {
		case 1:
			if (x1_[0] == 1) x1_ = from_straight_to_сomplementary(x1_);
			if (x2_[0] == 1) x2_ = from_straight_to_сomplementary(x2_);
			print(add(x1_, x2_));
			break;
		case 2:
